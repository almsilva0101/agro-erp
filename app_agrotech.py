...
# Função simples para criptografar senhas no banco (Segurança)
def com_hash(senha):
...
def inicializar_estruturas_nuvem():
    try:
        # 1. Garante que a tabela de usuários do ERP existe
        conn.query("""
        ...
        # Cria o admin padrão caso a tabela esteja vazia
        df_adm = conn.query("SELECT * FROM erp_usuarios WHERE usuario = 'admin';")
        if df_adm.empty:
            conn.query(f"INSERT INTO erp_usuarios VALUES ('admin', '{com_hash('admin')}', 'Administrador', '2026-06-09');")

        # 2. Garante que a tabela de frotas cadastradas via tela existe
        conn.query("""
...
# Leitura dos dados IoT e Operacionais
def ler_dados_nuvem():
    df_l = conn.query("SELECT * FROM lavoura;")
    df_p = conn.query("SELECT * FROM pecuaria;")
    df_r = conn.query("SELECT * FROM rotas;")
    
    # Tenta ler do seu simulador IoT rodando no CMD!
    try:
        df_f = conn.query("SELECT * FROM telemetria_tempo_real;")
        # Ajusta tamanho do ponto no mapa para os críticos
        df_f["size_map"] = df_f["status"].apply(lambda x: 30 if x == "CRÍTICO" else 12)
    except:
        # Caso o simulador esteja desligado, não quebra a tela
    ...
    with col2:
        ...
        if menu_auth == "Entrar no Sistema":
            ...
            if st.button("Autenticar no ERP", use_container_width=True, type="primary"):
                # Busca o usuário direto no Supabase
                df_user = conn.query(f"SELECT * FROM erp_usuarios WHERE usuario = '{user}';")
                if not df_user.empty and df_user.iloc[0]['senha_hash'] == com_hash(password):
                    st.session_state['logado'] = True
                    st.session_state['usuario'] = user
                    st.toast(f"🔑 Bem-vindo de volta, {user}!")
                    st.rerun()
                else:
            ...
            if st.button("Registrar na Nuvem", use_container_width=True, type="primary"):
                if not novo_user or not nova_senha:
                    ...
                    df_existe = conn.query(f"SELECT * FROM erp_usuarios WHERE usuario = '{novo_user}';")
                    if not df_existe.empty:
                        st.error("❌ Este nome de usuário já está em uso.")
                    else:
                        # Grava o novo usuário direto no banco de dados Supabase!
                        hash_db = com_hash(nova_senha)
                        data_atual = datetime.now().strftime("%Y-%m-%d")
                        conn.query(f"INSERT INTO erp_usuarios VALUES ('{novo_user}', '{hash_db}', '{novo_perfil}', '{data_atual}');")
                        st.success("🎉 Conta criada com sucesso! Mude para a aba 'Entrar no Sistema' para fazer login.")
...
def tela_dashboard():
    ...
    # KPIs Dinâmicos baseados no Supabase
    ...
    with tab_frota:
        if df_frota.empty:
            st.warning("⚠️ Aguardando dados do simulador_iot.py... Ligue o script no seu computador.")
        else:
            ...
            st.markdown("##### 📋 Painel de Sensores Ativos (Atualiza a cada 10s via Streamlit)")
...
def tela_cadastro():
    ...
    with st.form("form_cadastro_nuvem"):
        ...
        if st.form_submit_button("Sincronizar com Banco de Dados Cloud", type="primary"):
            if id_ativo:
                # Proteção contra SQL Injection básica (substitui aspas simples)
                id_clean = id_ativo.replace("'", "")
                # Executa o comando de INSERT direto na sua nuvem!
                try:
                    conn.query(f"INSERT INTO frota_manual VALUES ('{id_clean}', '{tipo}', '{fabricante}', '{chassi}', '{operador}');")
                    st.success(f"✅ Ativo {id_clean} gravado com sucesso no Supabase!")
                    st.rerun() # Atualiza a tela para puxar o dado novo
                except:
                    st.error("❌ Erro: Esse ID já existe no banco de dados.")
            else:
...
if not st.session_state['logado']:
    ...
    # Sidebar de Controle Interno do ERP
    ...
    if menu == "📊 Monitor C4 (Dashboard)":
        # Botão de atualização rápida na barra lateral
